...
class SPSA (object):
    ...
    def _gradient (self, params, loss, eps, args=()):
        """
        Calculates gradient for first-order SPSA
    ...
    def minimize (self, params,
        ...
        if np.linalg.norm(oldparams-params)/np.linalg.norm(oldparams)<self.precision:
            raise RuntimeError("Precision must be lower than 0.99,"
                                "but it's {}".format(self.precision))
